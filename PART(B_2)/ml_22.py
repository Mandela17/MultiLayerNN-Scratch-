...
class NeuralNetwork():
	...
	def tanh(self,s):
		return np.tanh(s)
	# def derivative_tanh(self, s):
	# 	#derivative of sigmoid
	# 	return s * (1 - np.power(s,2))
	def derivative_tanh(self,s):
		return (1.0 - (s**2))
	...
	def train(self, training_set_inputs, training_set_outputs, number_of_training_iterations):
		...
		for iteration in range(number_of_training_iterations):
			...
			# for i in range(len(training_set_inputs)):
				...
				a2 = self.tanh(dot(training_set_inputs[i], self.weights1))
				# print("a2")
				# print(a2)
				...
				del3 = dot(self.weights3, del4.T)*(self.derivative_tanh(a3).T)
				# print("del3")
				# print(del3)
				del2 = dot(self.weights2, del3)*(self.derivative_tanh(a2).T)
				# print("del2")
				# print(del2)
				...
				# print("fghj")
				# print(tts.shape)
				adjustment1 = dot(tts.T, del2)
				# print("adjustment1")
				# print(adjustment1)

				self.weights1 += adjustment1*learning_rate
				self.weights2 += adjustment2*learning_rate
				self.weights3 += adjustment3*learning_rate
			cost[iteration]=squared_error
	...
	def test(self,weights1,weights2,weights3,testing_set_inputs, testing_set_outputs, number_of_testing_iterations):
		...
		for iteration in range(number_of_testing_iterations):
			...
			# for i in range(len(testing_set_inputs)):
				...
				a2 = self.tanh(dot(testing_set_inputs[i], self.weights1))
				# print("a2")
				# print(a2)
				a3 = self.tanh(dot(a2, self.weights2))
				# print("a3")
...
if __name__ == "__main__":
	...
	ax.set_title('Tanh_soft_max in-sample error changes with Iterations in SGD')
	fig.savefig('Tanh_soft_max_in-sample error changes with Iterations in SGD.png')
	...
	ax.set_title('Tanh_softmax_out-of-sample error changes with Iterations in SGD')
	fig.savefig('Tanh_softmax_out-of-sample error changes with Iterations in SGD.png')
	...
	'''
	# print (neural_network.forward_pass(train_x[2],w1,w2,w3),train_y[2])
